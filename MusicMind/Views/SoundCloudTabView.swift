//
//  SoundCloudTabView.swift
//  MusicMindAI
//
//  Таб библиотеки SoundCloud
//

import SwiftUI

struct SoundCloudTabView: View {
    @ObservedObject var viewModel: SoundCloudLibraryViewModel
    ...
    private func emptyStateView(geometry: GeometryProxy) -> some View {
        VStack(spacing: 24) {
            ...
            VStack(spacing: 20) {
                PulsatingSphereView(
                    subtitle: soundCloudSubtitle,
                    subtitleColor: soundCloudSubtitleColor,
                    isActive: viewModel.isLoading || !authManager.isSoundCloudAuthenticated,
                    isUnauthorized: !authManager.isSoundCloudAuthenticated
                )
                
                if !authManager.isSoundCloudAuthenticated {
                    Button {
                        onAuthTap()
                    } label: {
                        Text("Войти")
        ...
        ScrollView {
            LazyVStack(spacing: 10) {
                ...
                ForEach(Array(filteredTracks.enumerated()), id: \.element.id) { index, track in
                    TrackRowView(track: track, index: index, service: .soundcloud, onTap: {
                        onTrackTap(track)
                    })
